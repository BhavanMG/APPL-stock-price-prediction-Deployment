...
# streamlit
st.title("Apple Stock price prediction using LSTM Model")
...
st.subheader("Apple Close price chart with 125 and 250 days moving avarage")
st.text('To analyse the Trend, it is common practice to use Moving Avarages.')
st.text('If 125 days moving avarage is greater then 250 days moving avarage, then it shows an UPTREND!')
st.text('If 125 days moving avarage is less then 250 days moving avarage, then it is shows DOWNTREND!')
ma_125 = df.Close.rolling(125).mean()
ma_250 = df.Close.rolling(250).mean()
fig = plt.figure(figsize=(12,6))
plt.plot(df.Close, label = 'Orifinal Close price')
plt.plot(ma_125,label='125 days Moving Avarage')
plt.plot(ma_250,label='250 days Moving Avarage')
#plt.xlabel("Date")
#plt.ylabel("Close price")
plt.legend(loc='best')
st.pyplot(fig)
...
st.subheader("Trained vs Predicted prices using LSTM")
# ploting results
...
st.subheader("Evaluation of LSTM Model")
st.subheader("test MAPE:")
st.subheader(mape(y_test,y_pred).round(2))
st.subheader("test RMSE:")
st.subheader(rmse(y_test,y_pred).round(2))
...
def pred_close(date1):
  if date1 in next_30.index:
    print(next_30['LSTM predicted Close price'].loc[date1])
  else:
    print("Given Date is a Holiday!")
#date_for = st.date_input('Give the Date you want to see Predicted Apple stock Price', pd.to_datetime('today'))
